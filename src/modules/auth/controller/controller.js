...
export const signup = async (req, res) => {
  ...
  if (user) {
    res.status(409).json({ message: "email is already exit" });
  } else {
...
export const confirmEmail = async (req, res) => {
  try {
    ...
    res.status(404).json({ message: "internal error", err });
  }
};
...
export const signin=async(req,res)=>{
    ...
    if(!user){
        res.status(400).json({message:"not register this account"})
    }else {
        if(!user.confirmEmail){
            res.status(400).json({message:"please confirmEmail"})
        }else {
